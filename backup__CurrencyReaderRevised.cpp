...
const double SMALLEST_AREA = 1500.0;
...
IplImage* contour(IplImage* src) {
	...
	for (CvSeq* c = first_contour; c != NULL; c = c->h_next) {
		...
		if (SMALLEST_AREA < area && area < BIGGEST_AREA) {

			cvSetImageROI(img, cvRect((int)box.center.x - (box.size.width / 2),
									  (int)box.center.y - (box.size.height/ 2),
									  (int)box.size.width,
									  (int)box.size.height));

			IplImage* canvas = cvCreateImage(cvGetSize(img),img->depth, img->nChannels);


			/* copy subimage */
			cvCopy(img, canvas, NULL);
